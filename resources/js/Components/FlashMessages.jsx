...
const ButtonClose = ({ color, onClick }) => {
  const className = cn('block  w-2 h-2 fill-current', {
  ...
  return (
    <div>
      {flash.success && visible && (
        <div className="mb-8 flex items-center justify-between bg-green-700 rounded">
      ...
      {(flash.error || numOfErrors > 0) && visible && (
        <div className="mb-8 flex items-center justify-between bg-red-700 rounded">
          <div className="flex items-center">
            <IconDanger />
            <div className="py-4 text-white text-sm font-medium">
              {flash.error && flash.error}
              {numOfErrors === 1 && 'Se encontro un error'}
              {numOfErrors > 1 && `Se encontraron ${numOfErrors} errores.`}
            </div>
          </div>
